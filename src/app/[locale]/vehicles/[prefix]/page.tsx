...
export default async function VehiclePrefixPage({
  params,
  searchParams
}: {
  ...
  const { locale, prefix: prefixRaw } = await params;
  const sp = await searchParams;

  // Don't intercept static sub-routes
  const staticRoutes = ['overview', 'new'];
  if (staticRoutes.includes(prefixRaw)) notFound();
  ...
  const [{ data: vehicles, count }, { data: branches }, { data: allPrefixes }] = await Promise.all([
    query,
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">
          {locale === 'th' ? `รถมอเตอร์ไซค์ (${prefixRow.label})` : `Motorcycles (${prefixRow.label})`}
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          {locale === 'th' ? `สต็อก ${prefixRow.label} — พร้อมขายและระหว่างซ่อม` : `${prefixRow.label} stock — available and under repair`}
        </p>
      </div>
      <VehicleTable
        locale={locale}
